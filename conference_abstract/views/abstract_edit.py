...
import time
from conference_abstract.services.data.abstract_info import getAbstract,getAbstractFullDetails
...
service = Service(name='abstractEdit', path='/abstract/{abstractId}/edit', description=info_desc)
    ...
    if login is not None:
        ...
        if user.is_admin() or user.is_reviewer():
            ...
            if abstracts is not None and len(abstracts)>0:
                ...
                return conference_abstract.util.generate_template('abstractReview.mako',{
                    'user': user,
            ...
            if abstracts is not None and len(abstracts) == 1:    
                abstract = abstracts[0]
                ## is this the submitter editing
                if user.userId == abstract["submitter"] and abstract["review_status"].upper() == "UNSUBMITTED":
                    categories, categoryNames = getCategories()
                    abstractAuthors = authors[int(abstractId)]
                    return conference_abstract.util.generate_template('abstractEdit.mako',{
                        'user': user,
                        'breadCrumbs':[{"url":"/","text":"Home"},{"url":"","text":"Abstract submisstion"}],
                        'abstract':abstract,
...
def service_post(request):
    ...
    if abstractTitle != "" and abstractText != "" and category != "":
        ...
        sql = """insert into abstracts (id, fk_submitter, title, abstract_text, selected_topics,submission_date,modification_date,abstract_type) values (%s, %s, %s, %s, %s, now(),now(),%s) RETURNING id;"""
