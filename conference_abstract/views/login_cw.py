...
service = Service(name='loginCw', path='/loginNew', description=info_desc)
...
def service_get(request):
    ...
    return conference_abstract.util.generate_template('loginNew.mako',{"user":user,"request":request,"pageTitle":"Login"})
...
def service_post(request):
    ...
    if 'submit' in request.POST:
        ...
        if user and loginCheck != False:
            ...
            print
            print "CWEI\t"
            print "user level:\t" , user.get_userLevel()
            print
            if user.get_userLevel() is None:     # ensure only reviewers & admins can login
                templateVars['message']="Sorry, access is now restricted to reviewers and conference coordinators"
            else:
    ...
    return conference_abstract.util.generate_template('loginNew.mako',templateVars)
