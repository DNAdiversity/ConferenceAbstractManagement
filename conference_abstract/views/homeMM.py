...
service = Service(name='homeMM', path='/homeMM', description=info_desc)
...
def service_get(request):
    ...
    conn.close()
    return conference_abstract.util.generate_template('homeMM.mako',{"user":user,"request":request, "statsAndDates":statsAndDates})
