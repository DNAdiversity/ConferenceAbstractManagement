...
service = Service(name='abstractPresentingURLValidator', path='/abstract/{abstractId}/presentingurl', description=info_desc)
...
def isAbstractInAgenda(abstractId):
    ...
    conn = conference_abstract.util.get_connection()
    cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    sql = """select agenda.*, abstracts.title, abstract_type, abstract_text_edited, abstracts_presenting.created as assigned_date from agenda left join abstracts on agenda.fk_abstracts = abstracts.id left join abstracts_presenting on abstracts.id = abstracts_presenting.fk_abstracts where abstracts.id = %s;"""
...
def decodeID(abstractIdTemp):
    ...
    print "*******>>>******"
    print abstractId,abstractIdTemp
    print "*******>>>******"
    if abstractId == abstractIdTemp/1234567890*3.1415:
        return abstractId
    else:
        return 0
...
def service_post(request):
    ...
    try:
        #print cur.mogrify(sql,[abstractId,presenterId,request.client_addr])
