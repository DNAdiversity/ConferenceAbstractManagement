...
service = Service(name='loginReviewer', path='/loginReviewer/{accessKey}', description=info_desc)
...
def service_get(request):
    ...
    try:
        ...
        if cur.rowcount == 1:
            ...
            user = User('chair@boldsystems.org')
            user.get_userObj()
            userObj = user.get_chairObj(accessKey)
            headers = pyramid.security.remember(request, 'chair@boldsystems.org'+"|"+str(accessKey)+"|"+str(userObj["chairId"]))
            session = request.session
            session['fullname'] = userObj["fullname"]
            session['userId'] = userObj["userId"]
