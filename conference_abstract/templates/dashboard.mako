...
<script type="text/javascript">
	$(document).ready(function(){
		...
		% if user.is_editor():
		$(".copyEditButton").on("click",function(ev){
			ev.preventDefault()
			var abstractId = $(this).attr("aid");
			window.location.href="/abstractCopyEdit/"+abstractId;
		});
		% endif
...
<%include file="includes/contentHeader.mako"/>
	...
	<div class="row">
		<div class="col-sm-12">
			...
			% if not user.is_admin() and not user.is_reviewer() and not user.is_editor():
			<div class="callout bordered">
				<span class="callout-icon reverse"></span>
				<div class="callout-wrapper">
					<div class="callout-left">
						<input type="button" class="btn btn-custom" value="Create New Abstract" onclick="location.href='/abstractNew'">
					</div><!-- End .callout-left -->
					<div class="callout-right text-right">
						<h2 class="callout-title">Abstracts are now being accepted until April 28, 2017!</h2>
			...
			<table class="table table-striped table-condensed">
				...
				<tbody>
					...
					<tr>
						...
						<td><strong>${abstract["title"].decode('utf-8')}
							<% 
							sep = "" 
							%>
							</strong>
						...
						% if user.is_admin() or user.is_reviewer():
							% if abstract["submitter_name"] is not None:
						<td>${abstract["submitter_name"].decode('utf-8')}</td>
							% else:
						<td> No submitter Name</td>
							% endif
						% endif
						...
						<span class="highlight 
							...
							% elif abstract['review_status'].lower() == 'assigned for edit':
							assignedForEdit
							% elif abstract['review_status'].lower() == 'rejected':
							rejected
							% else:
							first-color
							% endif
						...
						% elif user.is_editor():
						<span class="highlight 
							% if abstract['review_status'].lower() == 'assigned for edit': 
							third-color
							% elif abstract['review_status'].lower() == 'edited':
							rejected
							% else:
							first-color
							% endif
						...
						% if user.is_admin() or user.is_reviewer():
							% if abstract['review_status'] not in ["UNSUBMITTED","SUBMITTED"]:
						...
						<td>
							...
							<a type="button" value="" aid="${abstract['id']}" class="btn btn-info btn-sm reviewButton">Assign/Review<br>Copy Edit</a>
						</td>
						% elif user.is_reviewer():
						<td>
							##<select class="reviewer" aid="${abstract['id']}"></select>
						...
						<td>
							##<select class="reviewer" aid="${abstract['id']}"></select>
							<input type="button" value="Edit" aid="${abstract['id']}" class="btn btn-danger btn-sm copyEditButton"/>
						</td>
						% else:
			...
			% endif
			% if user.is_editor():
			<h2 class="title-border custom mb40">Copy Editor Directions</h2>
			...
			<li>Please replace non-English characters, and correct spacing, punctuation, spelling, and grammar in order to improve the abstract for publication.</li>
			<li>With the Rich Text Editor, please italize species names, and use other formatting such as line breaks and bolded text where appropriate.</li>
